...
export default function ProductPage() {
      ...
      for (let rowNumber = 2; rowNumber <= worksheet.rowCount; rowNumber++) {
        ...
        if (currentImage) {
          ...
          if (image?.buffer) {
            ...
            const imageRef = ref(
              storage,
              `humanbiomedicalin/products/${Date.now()}-${rowNumber}.png`
            );
  ...
  const handleMultipleImagesUpload = async (index, files) => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const imageRef = ref(
          storage,
          `humanbiomedicalin/products/${Date.now()}-${file.name}`
        );
  ...
  const handleVideoUpload = async (index, file) => {
    ...
    try {
      const videoRef = ref(
        storage,
        `humanbiomedicalin/videos/${Date.now()}-${file.name}`
      );
  ...
  const handlePdfUpload = async (index, file) => {
    ...
    try {
      const pdfRef = ref(
        storage,
        `humanbiomedicalin/pdfs/${Date.now()}-${file.name}`
      );
