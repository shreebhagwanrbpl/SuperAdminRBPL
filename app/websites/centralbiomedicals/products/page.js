...
export default function ProductPage() {
  ...
  const saveProducts = async () => {
    ...
    const docRef = doc(db, "websites", "centralbiomedicals", "pages", "products");
      ...
      for (let rowNumber = 2; rowNumber <= worksheet.rowCount; rowNumber++) {
        ...
        if (currentImage) {
          ...
          if (image?.buffer) {
            ...
            const imageRef = ref(
              storage,
              `centralbiomedicals/products/${Date.now()}-${rowNumber}.png`
            );
  ...
  const handleMultipleImagesUpload = async (index, files) => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const imageRef = ref(
          storage,
          `centralbiomedicals/products/${Date.now()}-${file.name}`
        );
  ...
  const handleVideoUpload = async (index, file) => {
    ...
    try {
      const videoRef = ref(
        storage,
        `centralbiomedicals/videos/${Date.now()}-${file.name}`
      );
  ...
  const handlePdfUpload = async (index, file) => {
    ...
    try {
      const pdfRef = ref(
        storage,
        `centralbiomedicals/pdfs/${Date.now()}-${file.name}`
      );
