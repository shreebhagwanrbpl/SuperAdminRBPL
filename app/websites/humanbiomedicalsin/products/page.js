...
export default function ProductPage() {
  ...
  const saveProducts = async () => {
    ...
    const docRef = doc(db, "websites", "humanbiomedicalsin", "pages", "products");
      ...
      for (let rowNumber = 2; rowNumber <= worksheet.rowCount; rowNumber++) {
        ...
        if (currentImage) {
          ...
          if (image?.buffer) {
            ...
            const imageRef = ref(
              storage,
              `humanbiomedicalsin/products/${Date.now()}-${rowNumber}.png`
            );
  ...
  const handleMultipleImagesUpload = async (index, files) => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const imageRef = ref(
          storage,
          `humanbiomedicalsin/products/${Date.now()}-${file.name}`
        );
  ...
  const handleVideoUpload = async (index, file) => {
    ...
    try {
      const videoRef = ref(
        storage,
        `humanbiomedicalsin/videos/${Date.now()}-${file.name}`
      );
  ...
  const handlePdfUpload = async (index, file) => {
    ...
    try {
      const pdfRef = ref(
        storage,
        `humanbiomedicalsin/pdfs/${Date.now()}-${file.name}`
      );
