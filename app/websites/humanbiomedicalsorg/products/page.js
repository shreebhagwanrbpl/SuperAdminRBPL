...
export default function ProductPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const snap = await getDoc(
        doc(db, "websites", "humanbiomedicalsorg", "pages", "products")
      );
  ...
  const saveProducts = async () => {
    ...
    const docRef = doc(db, "websites", "humanbiomedicalsorg", "pages", "products");
  ...
  const handleMultipleImagesUpload = async (index, files) => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const imageRef = ref(
          storage,
          `humanbiomedicalsorg/products/${Date.now()}-${file.name}`
        );
  ...
  const handleVideoUpload = async (index, file) => {
    ...
    try {
      const videoRef = ref(
        storage,
        `humanbiomedicalsorg/videos/${Date.now()}-${file.name}`
      );
  ...
  const handlePdfUpload = async (index, file) => {
    ...
    try {
      const pdfRef = ref(
        storage,
        `humanbiomedicalsorg/pdfs/${Date.now()}-${file.name}`
      );
