...
// Persistent & Memory cache for categories & subcategories
const _categoryCache = new Map();

export function setCachedCompanyCategories(companyId, categories) {
  if (!companyId || !Array.isArray(categories)) return;
  _categoryCache.set(companyId, categories);
  if (typeof window !== "undefined") {
    try {
      localStorage.setItem(`superadmin_categories_${companyId}`, JSON.stringify(categories));
    } catch (e) {}
  }
}
...
export function invalidateCompanyCategoriesCache(companyId) {
  if (companyId) {
    ...
    if (typeof window !== "undefined") {
      try {
        localStorage.removeItem(`superadmin_categories_${companyId}`);
      } catch (e) {}
    }
  } else {
    ...
    if (typeof window !== "undefined") {
      try {
        Object.keys(localStorage)
          .filter((k) => k.startsWith("superadmin_categories_"))
...
export function getCachedCompanyCategories(companyId, requireSubcategories = false) {
  if (!companyId) return null;
  let result = null;
  if (_categoryCache.has(companyId)) {
    const mem = _categoryCache.get(companyId);
  ...
  if (!result && typeof window !== "undefined") {
    try {
      const stored = localStorage.getItem(`superadmin_categories_${companyId}`);
      if (stored) {
        const parsed = JSON.parse(stored);
        if (Array.isArray(parsed) && parsed.length > 0) {
          _categoryCache.set(companyId, parsed);
          result = parsed;
        }
      }
    } catch (e) {}
  ...
  if (requireSubcategories) {
    const hasSubcategories = result.some(
...
export async function fetchCompanyCategories(companyId, includeSubcategories = true, forceRefresh = false) {
  ...
  if (!forceRefresh) {
    const cached = getCachedCompanyCategories(companyId, includeSubcategories);
  ...
  try {
    ...
    if (!snap.empty) {
      ...
      setCachedCompanyCategories(companyId, categories);
      return categories;
    }
  } catch (err) {
  ...
  if (fallbackCats && fallbackCats.length > 0) {
    setCachedCompanyCategories(companyId, fallbackCats);
  }
  return fallbackCats;
}
